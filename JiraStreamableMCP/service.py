"""Independent service facade for the Streamable HTTP MCP server."""
...
class JiraStreamableMCPError(RuntimeError):
    """Raised when the Streamable HTTP MCP service cannot complete a request."""


class JiraStreamableService:
    """HTTP-oriented Sprinter service facade.

    This class deliberately does not import or subclass ``MCPJira.service``.
    It is a separate adapter around the shared Sprinter core modules so the
    stdio MCP server and Streamable HTTP MCP server can evolve independently.
    """
    ...
    def server_info(self) -> Dict[str, Any]:
        ...
        return {
            "name": "JiraStreamableMCP",
            "transport": "streamable-http",
            "config_path": os.path.abspath(self.config_path),
            "resources": {
                "export_manifest": "jirastream://exports/{issue_key}/manifest",
                "created_response": "jirastream://created/{issue_key}/response",
            },
        }

    def export_issue(self, ticket_url: str) -> Dict[str, Any]:
        """Export a Jira issue and linked Confluence pages through HTTP MCP."""
        ...
        if not issue_key:
            raise JiraStreamableMCPError("Could not extract a Jira issue key from the provided URL.")
        ...
        try:
            ...
            write_manifest(issue_dir, manifest)
            raise JiraStreamableMCPError(str(exc)) from exc
        finally:
    ...
    def create_issue(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Create a Jira issue from a structured payload through HTTP MCP."""
        ...
        manifest = build_run_manifest("mcp+http://jira_create_issue", "pending")
    ...
    def _validate_ticket_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise JiraStreamableMCPError("Ticket payload must be a JSON object.")

        if "fields" not in payload and "update" not in payload:
            raise JiraStreamableMCPError("Ticket payload must include at least 'fields' or 'update'.")

        fields = payload.get("fields", {})
        if fields and not isinstance(fields, dict):
            raise JiraStreamableMCPError("'fields' must be a JSON object.")

        if "fields" in payload:
            for field_name in ("project", "issuetype", "summary"):
                if field_name not in fields:
                    raise JiraStreamableMCPError(f"Ticket payload is missing required field: fields.{field_name}")
    ...
    def _read_json(self, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise JiraStreamableMCPError(f"Artifact not found: {path}") from exc
        except json.JSONDecodeError as exc:
            raise JiraStreamableMCPError(f"Artifact is not valid JSON: {path}") from exc
